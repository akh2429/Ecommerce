    ...
    async function addHandler(id) {
        ...
        if (data.userId && data.productId) {
            ...
            if (response2.data === "Quantity Increased") {
                Swal.fire({ title: 'Sucess', text: `Quantity of item has been increased `, icon: 'success', confirmButtonText: 'Ok' });
    ...
    async function DecreaseHandler(id) {
        ...
        if (data.userId && data.productId) {
            ...
            if (response2.data === "Quantity Decreased") {
                Swal.fire({ title: 'Sucess', text: `Quantity of item has been decreased `, icon: 'success', confirmButtonText: 'Ok' });
                setCartUpdated(!cartUpdated);
    ...
    async function deleteHandler(id) {
        ...
        if (data.userId && data.productId) {
            ...
            if (response2.data === "Item Deleted") {
                Swal.fire({ title: 'Sucess', text: `Item sucessfully deleted `, icon: 'success', confirmButtonText: 'Ok' });
                setCartUpdated(!cartUpdated);
